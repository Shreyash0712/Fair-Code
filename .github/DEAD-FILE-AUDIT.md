...
- `assets/og-light/*.png` - the same set, rendered in the site's light theme instead of dark.
  Generated by the same `scripts/generate_og_images.py` pass as `assets/og/`, but not currently
  referenced by any `<meta>` tag - OG/Twitter's protocol has no equivalent of
  `prefers-color-scheme`, so a page can only declare one static `og:image`. Kept as a companion
  asset set (e.g. for manual social posts) rather than wired into `build_explainers.py`.
