...
```bash
python3 "Insurance Denial/unfair.py"   # biased baseline (protected attribute included)
python3 "Insurance Denial/fair.py"     # mitigated (protected attribute + proxies dropped)
```

## What the audit controls

- Protected attribute(s): Age, Gender
- Proxy feature(s) removed in `fair.py`: BMI, Smoker, Diabetic
