...
### Fixed
- **`scripts/generate_og_images.py`'s PNG output wasn't reproducible across platforms, repeatedly failing `Build Explainers` CI** - the same class of failure as #262's (a fresh explainer PR triggering the drift check against committed OG images), but this time the images were already correctly regenerated locally with CI's exact pinned Pillow (`11.1.0`) and still didn't match. Root cause: `img.save(..., optimize=True)` runs a per-scanline filter/strategy search whose "best" pick can differ by a few bytes between zlib builds - identical pixel content, different platform, different bytes - which a macOS-generated commit can never satisfy against CI's Ubuntu-side fresh regeneration no matter how many times it's retried. Switched to `compress_level=9`, a single deterministic deflate pass with no heuristic search; confirmed stable output across repeated local runs, and regenerated all 47 images.
