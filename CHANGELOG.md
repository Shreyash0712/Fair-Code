...
### Fixed
- **`build-explainers.yml`'s drift check compared PNGs by raw file bytes, which cross-platform zlib differences can never reliably satisfy** - the same class of failure as #262's, recurring even after the images were regenerated locally with CI's exact pinned Pillow (`11.1.0`). First tried switching `generate_og_images.py`'s `optimize=True` to a fixed `compress_level=9` (a single deterministic deflate pass instead of a heuristic filter/strategy search) - CI still failed, `0 insertions(+), 0 deletions(-)`, purely binary diffs, proving the mismatch survives even a fixed compression level: different zlib builds bundled into different platforms' Pillow wheels can produce different compressed bytes for pixel-identical input, full stop. The actual fix is `scripts/check_generated_files_current.py`: decodes each PNG (`git show HEAD:path` vs. the freshly regenerated working-tree file) and compares *pixel content*, not file bytes, while keeping byte-exact `git diff` for the text-based generated files (explainer HTML, `explainers-data.js`, `sitemap.xml`, `llms-full.txt`), which have never shown this problem. Verified both directions: swapping in a pixel-identical PNG re-encoded with entirely different compression settings (58,959 → 95,643 bytes) is correctly reported as up to date; a real one-pixel content change is still correctly caught and fails.
