...
def compare_to_html(cmp: dict) -> str:
    ...
    delta_class = "up" if score_delta > 0 else ("down" if score_delta < 0 else "flat")
    arrow = "=" if score_delta == 0 else "→"

    summary_html = (
        '<div class="drift-summary">'
        f'<div class="drift-score"><span class="n">{a["overall_score"]}</span><span class="l">{esc(a["name"])} ({a["n_rows"]:,} rows, Grade {a["grade"]})</span></div>'
        f'<div class="drift-arrow" aria-hidden="true">{arrow}</div>'
        f'<div class="drift-score"><span class="n">{b["overall_score"]}</span><span class="l">{esc(b["name"])} ({b["n_rows"]:,} rows, Grade {b["grade"]})</span></div>'
        f'<div class="drift-delta {delta_class}">score {signed(score_delta, 0)} pts</div>'
        '</div>'
    )
